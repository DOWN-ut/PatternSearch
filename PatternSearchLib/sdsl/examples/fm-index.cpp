...
int main(int argc, char** argv)
{
	...
	ifstream source("file/message.txt");

    char inter[20];
    source.ignore(1000,'\t'); //ignore nom
    char inter_i[100];
    source.getline(inter_i,100,'\t');
    int taille = atoi(inter_i);
    
    
    char inter_nb[100];
    source.getline(inter_nb,100,'\t');
    ...
    int test=0;
    for (int j=0; j<nb;j++){// boucle mot
    query.erase();
    source.get(inter,taille +1);
   
  
    
    int i=0;
    while(inter[i]){  // char[] to string
    query+= inter[i];
	i++;
    }
    				//recherche
